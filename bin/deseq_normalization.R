...
if (length(args) != 2) {
  stop(
    "Please input the location of the sample information file, and the gene_counts table",
    call. = FALSE
  )
}

# Parse sample informations
condition = read.csv(file = args[1],
                     sep = ",",
                     header = T)$condition
...
if (length(unique(condition)) == 1) {
  ...
  ggsave(
    "PCA.pdf",
    plot = plotPCA(vsd, intgroup = c("condition")) + geom_text(
      aes(label = name),
      vjust = 2,
      check_overlap = TRUE,
      size = 3
...
write.table(
  ...
  file = "counts_gene.normalized.vsd.csv",
  quote = FALSE,
  sep = "\t"
)
